    ...
    <div className="h-full overflow-y-auto p-4 space-y-3" role="log" aria-live="polite" aria-relevant="additions" aria-label="Chat messages">
        {messages.map((msg) => {
          ...
          return (
            <div key={msg.id} className={`flex items-start gap-3 ${msg.sender === 'user' ? 'justify-end' : ''}`}>
              <div className={`flex-shrink-0 w-8 h-8 rounded-full grid place-items-center text-sm font-semibold ${msg.sender === 'user' ? 'bg-violet-600 text-white' : 'bg-gray-700 text-gray-100'}`}>{msg.sender === "user" ? "U" : "🤖"}</div>
        ...
        {loadingBot && (
          <div className="flex items-start gap-3">
            <div className="flex-shrink-0 w-8 h-8 rounded-full grid place-items-center bg-gray-700 text-gray-100">🤖</div>
